...
CREATE OR REPLACE PROCEDURE process_metric_data(start_ts INT, end_ts INT, host_st INT, host_en INT) 
AS $$
DECLARE
    rec record;
    pid INT;
...
BEGIN
    ...
    FOR rec IN
        SELECT i FROM generate_series(0,100) i ORDER BY random()
    LOOP
        raise notice '% create metric: %', pid, 'bar_foo' || rec.i;
        PERFORM create_metric_table('foo' || rec.i, 'bar', 'FLOAT8', 1, 3600, 604800, 2592000); 
        COMMIT;
        CALL finalize_metric_creation('foo' || rec.i, 'bar');
        COMMIT;

        raise notice '% generate data: %', pid, 'bar_foo' || rec.i;
        PERFORM insert_metric_row(('bar_foo' || rec.i)::name, ts_arr, val_arr, series_arr)
        FROM (
            SELECT array_agg(ts) ts_arr, array_agg(3.14::float8) val_arr, array_agg(series_id::int) series_arr 
            FROM (
                SELECT ts FROM generate_series(start_ts, end_ts) ts
            ) timez
            CROSS JOIN (
                    SELECT (get_or_create_series_id_for_kv_array('bar', 'foo' || rec.i, ARRAY['hostname'], ARRAY['host' || host_idx])).* FROM generate_series(host_st,host_en) host_idx 
            ) seriez
            ORDER BY random()
        ) dat;
        ...
        raise notice '% done: %', pid, 'bar_foo' || rec.i;
    END LOOP;
END
$$
LANGUAGE PLPGSQL;
...
CREATE OR REPLACE PROCEDURE process_metric_data_alt(start_ts INT, end_ts INT, host_st INT, host_en INT) 
AS $$
DECLARE
    rec record;
    pid INT;
...
BEGIN
    ...
    LOOP
        ...
        SELECT array_agg(ts) ts_arr, array_agg(3.14::float8) val_arr, array_agg(series_id::int) series_arr INTO tss, vals, seriezz
        FROM (
            SELECT ts, 3.14, series_id
            FROM (
                SELECT ts FROM generate_series(start_ts, end_ts) ts
            ) timez
            CROSS JOIN (
                SELECT DISTINCT id as series_id FROM series WHERE metric_id = (SELECT id FROM metric WHERE metric_name = 'foo' || rec.i)
            ) seriez
            ORDER BY random()
        ) glah;

        SELECT array_length(tss, 1) INTO arlen;
        j := 1;

        WHILE j < arlen
        LOOP
            raise notice '% insert data: %', pid, 'bar_foo' || rec.i;
            IF j+14 >= arlen THEN
                PERFORM insert_metric_row(('bar_foo' || rec.i)::name, tss[j:], vals[j:], seriezz[j:]);
            ELSE
                PERFORM insert_metric_row(('bar_foo' || rec.i)::name, tss[j:j+14], vals[j:j+14], seriezz[j:j+14]);
            END IF;
